...
template<typename Vec, typename Mat> Vec foo() {
  Mat G(3,3); 
  Vec g0(3);
  Mat CE(3,1);
  Vec ce0(1);
  ...
  std::cout << "f: " << solve_quadprog_1(G, g0,  CE, ce0,  CI, ci0, x) << std::endl;
  std::cout << "x: ";
...
int main(int argc, char** argv){
  Eigen::VectorXd x; 
  x = foo<Eigen::VectorXd, MatrixXd>();
  
  std::cout << "printing x again" << std::endl;
